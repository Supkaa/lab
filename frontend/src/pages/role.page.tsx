...
const RolePage = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3333/roles', {headers: {Authorization: `Bearer ${token}`}}).then((response) => {
            setRoles(response.data);
        })
        axios.get('http://localhost:3333/endpoints', {headers: {Authorization: `Bearer ${token}`}}).then((response) => {
    ...
    return (
        <div className={'flex flex-col items-center'}>
            <div className="relative overflow-x-auto mt-28 w-1/2">
                    ...
                    {roles?.map((role) => (
                        <tr key={role.name} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td className="px-6 py-4 flex gap-4">
                                <button
                                    onClick={() => {
                                        axios.delete(`http://localhost:3333/roles/${role.name}`).then(() => {
                ...
                {roleId && (
                    <div className={'flex gap-7 mt-4'}>
                        ...
                        <button
                            onClick={() => {
                                axios.put(`http://localhost:3333/roles/${roleId}`, {
                                    endpointIds: ids
            ...
            <div className={'flex gap-7 mt-4 w-1/2'}>
                ...
                <button
                    onClick={() => {
                        axios.post(`http://localhost:3333/roles`, {
                            name: ids,
            ...
            <table className="w-1/2 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mt-4">
                ...
                {endpointUrl?.map((endpoint) => (
                    <tr key={endpoint.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {endpoint.id}
                        </td>
                        <td className="px-6 py-4 flex gap-4">
